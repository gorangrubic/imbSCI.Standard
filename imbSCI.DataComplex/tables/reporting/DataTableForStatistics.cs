...
namespace imbSCI.DataComplex.tables
{
    ...
    public class DataTableForStatistics : DataTable, IDataTableForStatistics
    {
        ...
        /// <summary>
        /// Declares non-data rows to be displeyed before and after data rows (on main spreadsheet / view)
        /// </summary>
        /// <value>
        /// The extra row styles.
        /// </value>
        ...
        public DataTable RenderDataTable()
        {
            ...
            var columnStatistics = output.GetColumnSetupStatistics();

            List<templateFieldDataTable> existingSetupSetters = columnStatistics.GetDistinctItems();
            

            // output.AddRowDescriptionColumn("Row info", true);
            // <--- ovde ubaciti da atributi klase odredjuju stra prikazuje
            if (existingSetupSetters.Contains(templateFieldDataTable.col_group)) extraRowStyles.Add(output.AddExtraRow(templateFieldDataTable.col_group, 200), DataRowInReportTypeEnum.mergedCategoryHeader);
            if (existingSetupSetters.Contains(templateFieldDataTable.col_caption)) extraRowStyles.Add(output.AddExtraRow(templateFieldDataTable.col_caption, 200), DataRowInReportTypeEnum.columnCaption);
            if (existingSetupSetters.Contains(templateFieldDataTable.col_unit)) extraRowStyles.Add(output.AddExtraRow(templateFieldDataTable.col_unit, 200), DataRowInReportTypeEnum.columnDescription);
            if (existingSetupSetters.Contains(templateFieldDataTable.col_letter)) extraRowStyles.Add(output.AddExtraRow(templateFieldDataTable.col_letter, 200), DataRowInReportTypeEnum.columnDescription);
        ...
        public void RenderToLegendWorksheet(ExcelWorksheet ws)
        {
            DefineStyles(ws);
        ...
        public void RenderToWorksheet(ExcelWorksheet ws)
        {
            DefineStyles(ws);
