...
namespace imbSCI.Core.reporting.render.builders
{
    public class builderForGutenberg : imbStringBuilderBase, ITextRender
    {
        ...
        public override void AppendDirect(string content)
        {
            _Append(content, true);
            //base.AppendDirect(content);
        }
        ...
        {
            if (table.TableName != table.GetTitle())
            {
                AppendHeading(table.GetTitle(), 3);
            }

            String htmlTable = imbStringHTMLExtensions.htmlTable(table, "wp-block-table is-style-stripes", false);
            String c = "<!-- wp:table {\"className\":\" is-style-stripes\"} -->" + htmlTable + "<!-- /wp:paragraph -->";
            _AppendLine(c);

            if (table.ExtraLinesCount() > 0)
            {
                AppendCite(table.GetExtraDesc().toCsvInLine(Environment.NewLine));
            }
